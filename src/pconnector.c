// http://phhusson.free.fr/miniprogs/cn-proc.c.html
...
int pconnector_init(void)
{
    ...
    if ((sock_nl = socket(PF_NETLINK, SOCK_DGRAM, NETLINK_CONNECTOR)) == -1)
        return -1;
    ...
    if (bind(sock_nl, (struct sockaddr *)&my_nla, sizeof(my_nla)) == -1) {
        close(sock_nl);
        return -2;
    }
    ...
    *mcop_msg = PROC_CN_MCAST_LISTEN;
    ...
    if (send(sock_nl, nl_hdr, nl_hdr->nlmsg_len, 0) != nl_hdr->nlmsg_len) {
        close(sock_nl);
        return -3;
    }
    ...
    for(memset(buff, 0, sizeof(buff)), from_nla_len = sizeof(from_nla);
      ; memset(buff, 0, sizeof(buff)), from_nla_len = sizeof(from_nla)) {
        ...
        recv_len = recvfrom(sock_nl, buff, BUFF_SIZE, 0,
